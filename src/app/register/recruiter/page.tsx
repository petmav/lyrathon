      ...
      <main className="panel" style={{ alignItems: 'center', justifyContent: 'center' }}>
        <div ref={heroRef} className="container" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "60px", alignItems: "center" }}>
          ...
          <div className="hero-content" style={{ padding: 0 }}>
            <p className={`eyebrow reveal ${heroSeen ? "show reveal-delay-0" : ""}`}>Recruiter Workspace</p>
            <h1 className={`hero-title reveal ${heroSeen ? "show reveal-delay-1" : ""}`}>
              Hire with precision. <br /> Not volume.
            </h1>
            <p className={`hero-subtitle reveal ${heroSeen ? "show reveal-delay-2" : ""}`}>
              Access a pre-vetted talent pool where 82% of candidates engage within 24 hours.
              Use natural language to build shortlists in seconds.
            </p>

            {/* KPIs */}
            <div className={`hero-kpis reveal ${heroSeen ? "show reveal-delay-3" : ""}`} style={{ marginTop: 32 }}>
              <div className="kpi-card">
                <span className="kpi-value" data-target="3" data-suffix="x">0x</span>
                <span className="kpi-label">Faster screening</span>
              </div>
              <div className="kpi-card">
                <span className="kpi-value" data-target="92" data-suffix="%">0%</span>
                <span className="kpi-label">Placement successful</span>
              </div>
              <div className="kpi-card">
                <span className="kpi-value" data-target="450" data-suffix="+">0+</span>
                <span className="kpi-label">Startups hiring</span>
              </div>
            </div>
          </div>
          ...
          <div className={`glass-card reveal ${heroSeen ? "show reveal-delay-2" : ""}`} style={{ padding: 40, width: '100%', maxWidth: 480, margin: '0 auto' }}>
            <div style={{ marginBottom: 32 }}>
              <h2 style={{ fontSize: '1.8rem', fontWeight: 700, margin: 0 }}>Create workspace</h2>
              <p className="muted">For recruiters and hiring managers.</p>
            </div>
